...
This closes the render-path gate's two owed items — (d) the live cookie-into-frame run, and (f) whether
LibreChat honours `notifications/tools/list_changed`. Items (a), (b), (c), (e) were answered
earlier by source read and are re-asserted here against the installed artifacts
rather than against GitHub.

Every LibreChat bump requires re-verification. The harness is committed at
...
and share the registrable domain `noa.internal` — the deployment's registrable-parent topology.
...
- `document.cookie` → empty. The session cookie is httpOnly and stays unreadable.
...
`httpOnly=true`, `sameSite=Lax`, `secure=false` (plain-HTTP rig). So the decision POST — cookie
plus CSRF, never the LLM — is possible from inside the iframe, by JS `fetch`, with the operator's session.

`allow-forms` was absent from **both** sandbox strings, so the fetch-only in-frame rule holds
live, not just by source read: a native `<form>` submit is not available in-frame, and the
approve/deny POST must be a `fetch`.
...
That is the embed-on-NOA-origin design's failure mode, observed. It is also what makes the PASS above a measurement
...
Consequence for the list-changed emitter: emitting the notification is not harmful (the client ignores it without
erroring or dropping the connection), but it buys nothing at this pin. A revoked tool can stay
in LibreChat's per-user catalog until that connection is rebuilt. That is no real hole: every
request re-checks permission at execution, so the stale entry is a display artifact, not an
authorization hole.

## Findings that are not about rendering

Three things the run surfaced that bear on the LibreChat config doc (the `librechat.yaml` NOA entry) and were not
...
1. **A header-authenticated MCP server gets marked OAuth-required at boot.** LibreChat
   ...
   nothing to resolve against and NOA answers 401 — correctly, since LibreChat is the sole
   client and the 401 body already names the cause. LibreChat reads
   ...
   from LibreChat being the sole client.
...
3. **The resource id and marker are LLM-visible.** LibreChat's parser
   ...
   \ui{<id>}` into the text the model sees — which is what the model-facing safety text's
   `\ui{}` instruction is for. Nothing about the reason or the decision travels there (the
   operator-only reason rule untouched), but it confirms the approval URL and id land in
   LibreChat's MongoDB.
...
- **Probe tools are not NOA tools.** They live on a second FastMCP server at
  ...
  `register_mcp_tools` refuses any name outside `TOOL_CATALOG` — the catalog gate
  that should stop a throwaway tool from entering the real registry. Consequently the run
  exercised token auth and TOFU binding but **not** `RbacToolMiddleware` or
  `ToolRunAuditMiddleware`.
- **`/embed-probe/decide` is not the approve endpoint.** It authenticates the caller through
  production's own `require_session_user` and answers; it implements no CSRF token, no
  row lock, no reason. The real one is
  `POST /action-requests/{id}/approve` (the request-opening gate writes the row, the decision
  endpoint decides it) — this rig
...
**`autoResizeIframe` still passed, with both axes enabled, at all three render sites.** The three render sites
(`MCPUIResource.tsx`, `ToolCallInfo.tsx`, `UIResourceCarousel.tsx`) each pass an
...
`autoResizeIframe`.** The same rule applies here as to the script's
existing `text/uri-list` and `allow-forms` checks: upstream provenance is not evidence a
control works, so a control needs a test against the real mechanism. That rule's literal subject
is the ported SSH host-key control; it is being applied one surface over here, to shipped bytes
versus the source they were published from. The real mechanism is what npm installed, so both
assertions run against `node_modules/@mcp-ui/client/dist/index.mjs`, the bytes LibreChat
actually loads, not mcp-ui's repo source.
