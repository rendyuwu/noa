...
- `repository` — opens a request. `SQLActionRequestRepository` writes `PENDING` and can write
  nothing else, because its caller is the MCP tool path — the path an LLM can reach.
...
- `delta` — the vocabulary a runner states its own before→after in, and the one module here
  ...
  optional in the type, so an absence is structural rather than remembered, and a
  reason-bearing key is refused at construction.
- `reaper` — resolves what nobody finished. Runs left `STARTED` past a deadline
  ...
  transaction and a capped pass never reads like a complete one.
- `expiry` — answers one that nobody answered. `SQLActionRequestExpiryRepository` can
...
- `results` — reads one back *for a model*: `noa_get_action_result`. Structurally
  ...
  put any of them.
- `card` — reads one back *for the operator in front of it*: the approval card's
...
needs the caller's identity and the in-process preflight evidence. Deciding one is
`noa_api.api.routes.action_requests`. Reading one back over MCP is
`noa_api.mcp_tools.noa_read`, which supplies the caller `results` matches against; over
HTTP it is the `GET` on that same decision router, which supplies `card`'s.
