...
Ported, not imported, from `noa-old` branch `MCP` (`core/secrets/redaction.py`) — the key list
and the recursive walk are the source's, and what the walk does is unchanged. Its mapping branch
is a named function here (`redact_mapping`), for a caller that holds a mapping and would
otherwise have to assert its way back to one. The port deliberately left this module behind at
first: it lands with the code that *writes* redacted audit args, the tool-run writer, because a
redactor with no caller is a control no test exercises, and that is how the inert host-key pin
shipped.
...
admin UI could decrypt them back. The run-row audit rule says **redacted**, and an encrypted
argument is recoverable — a Fernet key compromise turns the whole audit table into a credential
dump.
...
# `noa-old`'s list verbatim. It covers the credential columns of schema v1 (`api_token`,
# `api_token_secret`, `ssh_*`) and the argument names of the CHANGE tools that carry a secret
# (`cipassword` for the password-reset cloud-init write). Comparison is case-insensitive and
# whitespace-stripped, so a schema that spells one differently still matches.
...
def redact_sensitive_data(value: object, *, replacement: str = REDACTED) -> object:
    ...
    Recurses through mappings and sequences so a credential nested inside a structured argument is
    caught too — a flat top-level scan would miss `{"server": {"ssh_password": ...}}`.
...
def redact_mapping(value: Mapping[str, Any], *, replacement: str = REDACTED) -> dict[str, Any]:
    ...
    redactor that answered something else, which is a benign value standing in for a bug — a
    non-answer folded into the good value, one surface over. Naming the mapping case is the third
    option, and it costs nothing: the two functions are one policy, and a key comparison changed
    here changes both.
    """
    return {
        str(key): (
...
def sensitive_key_paths(value: object, *, _prefix: str = "") -> list[str]:
    ...
    has to *refuse* a redacted payload rather than produce one (the approved-change executor,
    which will not run a change whose arguments came back as `[redacted]`) asks the same question
    the
