...
Copied from `noa-old` branch `MCP` (`noa_api/proxmox/integrations/`) — port, never rewrite.
One transport, unlike WHM: Proxmox exposes everything NOA needs over its HTTP API,
so there is no SSH path here and a `proxmox_servers` row carries no SSH credentials.
...
- `client`    — `ProxmoxClient` + `build_proxmox_client_from_creds` / `build_proxmox_client`.
                ...
                `permission_denied` (fix the ACL in Proxmox, never the token in NOA) and
                `digest_mismatch` (re-read, never retry).
...
Proxmox needs no CLI over SSH. `SecretCipher` is injected rather than imported —
