...
`gather()` returned `[]`, and the tool reported success having changed nothing. V57 forbids
that, and T24 refused it — but with a check written *inside* the preflight tool, which is a
guard the next tool can forget. A CHANGE tool that forgets it is the harm V57 is actually
about: an approved suspension or firewall release that reports done and did nothing, on a
server NOA could not drive at all.

So the fan-out itself refuses the empty set. `run_on_usable_backends` is the only place a
backend set becomes concurrent work, and it cannot be entered with nothing to do. T25
(`whm_firewall_release_and_allow`) and T26 (`whm_firewall_allowlist_remove`) inherit the guard
...
`asyncio.gather` from quietly re-opening the hole (V84c: a machine-readable property is bound
by a test, not by a docstring).

**Two codes, because two remedies** (`noa-old` GH #82, V55). A denied `sudo -n` means a
...
def usable_backends(availability: FirewallAvailability) -> tuple[str, ...]:
    ...
    identical calls have to produce the same result (V85's reproducibility clause).
    """
