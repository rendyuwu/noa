...
import AdminAuditToolRunsRoute from './page'
...
describe('/admin/audit/tool-runs route gate', () => {
  it('re-verifies with requireAdmin so the server decides access', () => {
    render(<AdminAuditToolRunsRoute />)
    expect(state.lastOptions).toEqual({ requireAdmin: true })
  })

  it('deep-links into the tool-runs tab for a verified admin', () => {
    render(<AdminAuditToolRunsRoute />)
    expect(screen.getByTestId('audit-page')).toHaveTextContent('tool-runs')
  })
