...
vi.mock('@/components/admin/pmg/pmg-servers-page', () => ({
  PmgServersPage: () => <div data-testid="pmg-page">pmg</div>,
}))

import AdminPmgRoute from './page'
...
describe('/admin/pmg route gate', () => {
  it('re-verifies with requireAdmin so the server decides access', () => {
    render(<AdminPmgRoute />)
    expect(state.lastOptions).toEqual({ requireAdmin: true })
  })

  it('renders the PMG page for a verified admin', () => {
    render(<AdminPmgRoute />)
    expect(screen.getByTestId('pmg-page')).toBeInTheDocument()
  })
  ...
  it('shows a loading placeholder while verifying', () => {
    ...
    expect(screen.queryByTestId('pmg-page')).not.toBeInTheDocument()
  })

  it('renders nothing on an unexpected auth error', () => {
    state.value = { status: 'error', user: null, isAdmin: false }
    const { container } = render(<AdminPmgRoute />)
