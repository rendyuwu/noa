 ...
 * a four-entry allowlist because the embed is the one NOA origin LibreChat may frame (V41): a
 ...
 * same pass-through for the same app (C13).
 *
 * What keeps that width safe lives in `http.ts` and is tested there: `Authorization` is dropped
 * outbound (C5), so `/api/mcp/` is reachable but inert; a traversal cannot escape the base prefix;
