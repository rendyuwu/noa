 ...
 * an admin panel silently talking to the wrong API is not a failure anyone would see. So the
 * repo-root file is the single source, read here and applied by `next.config.ts`.
 *
 ...
 *
 * The precedence is load-bearing: a real shell or CI variable wins, which is what lets a test run
 * point `NOA_API_URL` at its own stub upstream instead of whatever a developer's `.env` says.
 * No-op when there is no `.git` above this package or no `.env` beside it — production supplies env
 * via ConfigMaps/Secrets, and `output: 'standalone'` never executes the Next config at runtime
 * anyway.
 */
