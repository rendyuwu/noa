...
beforeEach(() => {
  ...
  app = path.join(root, 'apps', 'admin-web')
...
describe('loadRootEnv', () => {
  ...
  it('never overwrites a key already set in the real environment', () => {
    // Load-bearing, not a nicety: CI and a deployment both hand `NOA_API_URL` in as a
    // real environment variable. If the file won, a checked-in developer default would
    // silently outrank the API the environment was pointed at.
    writeFileSync(path.join(root, '.env'), 'NOA_API_URL=http://localhost:8000\n')
  ...
  it('treats a key set to empty string as set — `in` is the test, not truthiness', () => {
    ...
    // would silently re-point the app at an API they turned off.
