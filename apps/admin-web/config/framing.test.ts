 ...
 * itself. Nothing is read from the environment here, so unlike the embed's
...
describe('buildFramingHeaders', () => {
  ...
  it('covers every path, not just the pages', () => {
    ...
    // covers the pages, `/healthz`, the 404 and the `/api/*` proxy adds. A page-shaped
    // pattern would leave each new route to remember the guard for itself (the one-place-not-per-route shape).
    expect(FRAMING_SOURCE).toBe('/(.*)')
    expect(buildFramingHeaders()).toHaveLength(1)
  })

  it('sends no X-Frame-Options', () => {
    // The embed's own framing-header test measured the same thing: `frame-ancestors` supersedes XFO wherever both
    ...
    // Literal, not `FRAMING_HEADER.toLowerCase()`: measured on the embed — pointing the
  ...
  it('takes no configuration — there is no origin to name', () => {
    // The separating case against the embed's shape, where the origin is an env variable
