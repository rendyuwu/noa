 ...
 *
 * The 401 case reaches §T.43's way out, which this surface reuses rather than reimplements (V66):
 * a link *and* the same address as text, because the render site whose sandbox omits `allow-popups`
 * opens the link silently (V94, R32). `sign-in.browser.e2e.ts` owns the two-sandbox measurement of
...
test('the rows render inside the measured sandbox (V64)', async ({ page }) => {
...
test('a capped page says so, in the frame (V85)', async ({ page }) => {
...
test('an uncapped page states its total without claiming truncation (V87)', async ({ page }) => {
...
test('the operator’s cookie reaches the API through the page’s own read (V22, V40)', async ({
  page,
}) => {
  ...
  // that cannot fail for the reason it exists (V87).
  //
  ...
  // `noa.internal` (V40) and the cookie is same-site, which is what R29 measured live against
...
test('a 401 renders the way out, with the address as text beside the link (V38, V94)', async ({
  page,
}) => {
  ...
  // The door the sandbox cannot withhold: the same address, as copyable text (V94, R32).
...
test('a 404 renders one sentence for every cause, and never an empty table (V27, V38)', async ({
  page,
}) => {
