...
/**
 * A page that puts a document in an iframe, for the framing and approval specs.
 *
 ...
 * response, so the refusal spec would have passed without a CSP header existing at all (the
 * setup-gate rule's family: the setup step deciding the outcome).
 *
 * **`?src` and `?sandbox` are how the approval card borrows this** rather than standing up a
 * second parent. The approval specs frame the card under the sandbox string LibreChat was
 * measured applying at pin `45cc53c4` — `allow-scripts allow-same-origin`, with `allow-forms`
 * absent — which is the whole premise the JS-fetch-only rule rests on. Both parameters default
 * to the fixed frame's default, so those specs are unaffected by the addition.
 *
...
/** The box the fixed frame opens with, unchanged so the specs that predate `?height` are. */
