...
/**
 * Loading one approval card, server-side.
 *
 ...
 * allowlisted proxy, which is the path the cookie-POST-plus-CSRF rule names.
 *
 ...
 * and this loader adds no header a caller could use to relay one — the same departure the
 * proxy route makes.
 *
 * **Four outcomes, because four of them render differently** — `ApprovalCardLoad` says which, and it
 * lives in `lib/approvals/card.ts` because the browser-side poll answers the same four
...
function upstreamUrl(actionRequestId: string): string {
  ...
  // backend's base prefix. The id is not shape-checked here: the requester-match rule owns what
  // an absent, malformed or foreign id answers and answers all three alike, so a UUID test in
...
export async function loadApprovalCard(
  ...
  // fields as blanks would put an Approve button on top of nothing — the same failure a blank
  // 401 card would be.
