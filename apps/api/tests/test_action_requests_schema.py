...
- **Metadata** — the column set, the enum definition, the indexes, and the absences the
  reason rule turns on. No DB needed.
...
This file is about the *shape*, not the writer. The gate inserts the row, the decision
endpoints decide it, the executor links the run and the sweeper expires it; what is asserted
here is that the columns those writers need exist and hold, so none of them can quietly
reshape the table.
"""
...
# The table's column list, verbatim. A column added without a writer to specify it is a guess
# (`test_schema_v1.py` guards whole tables the same way); one removed takes a lifecycle field
# with it.
EXPECTED_COLUMNS = {
    "id",
    "tool_name",
...
# What the table's design says must NOT be here, plus the four `noa-old` carried that this
# design refuses. Named one by one so a failure says which boundary was crossed.
FORBIDDEN_COLUMNS = {
    # The reason is operator-typed at approve time. `noa-old` had no such column
    # either, but it did worse — the reason rode inside `args`, so the LLM authored it.
    "proposed_reason": "the LLM never authors, relays or sees a reason",
    # One gate-time payload, one record of one moment.
    "args": "arguments live inside `approval_context`",
    # READs never reach the gate, so every row here is a CHANGE.
    "risk": "risk lives on `tool_runs`, which is what makes a failed READ writable",
    # The decider is the requester; a mismatch is a 404.
    "decided_by_user_id": "one identity, not two truths about one person",
    # One mutation is permitted and `decided_at` stamps it.
    "updated_at": "`decided_at` is the only mutation timestamp",
}

# Nothing filters this table the way the admin API filters the audit surface, so the index
...
def test_column_set_is_exactly_what_t34_specifies() -> None:
    ...
    assert set(ACTION_REQUESTS.c.keys()) == EXPECTED_COLUMNS
...
def test_exactly_one_reason_column_exists() -> None:
    """Exactly ONE reason field, and this is it.

    Not "a reason column exists" — *one*. A second spelling anywhere on the row is what
    the one-field rule forbids, and `proposed_reason` was only the name `noa-old`'s design
    implied; the assertion is over the whole column set so a differently-named twin fails too.
    """
...
def test_no_table_in_the_schema_carries_a_proposed_reason_column() -> None:
    """The one-field rule across the whole schema, not just this table.

    Written over `Base.metadata` on purpose: `action_receipts` and every later table
    inherit the rule without being listed here, the same way `test_config.py` scans every
    tracked file rather than an enumerated set. The rule is "never an LLM-authored reason
    anywhere in schema/DB/receipt", and this assertion covers the receipt half too —
...
def test_reason_is_nullable_and_unbounded() -> None:
    """NULL until an operator types one; still NULL after an expiry.
    ...
    truncating the field that authorises a change is worse than storing a long one. The
    decision endpoints bound the input.
    """
...
def test_action_request_status_is_exactly_the_four_members() -> None:
    """A machine-stable member set, `EXPIRED` included.
    ...
    forever and the TTL had no terminal state to sweep into.
    """
...
def test_the_three_lifecycle_enums_are_distinct_with_disjoint_values() -> None:
    """Three distinct enums, not one flat set — and now all three exist.
...
def test_status_defaults_to_pending() -> None:
    ...
    A server-side default, not an ORM-side one: this column is read as the authorization,
...
def test_approval_context_is_required_and_has_no_default() -> None:
    """Built at gate time and persisted here, never rebuilt at render time.
...
def test_expires_at_is_required_so_every_pending_row_carries_a_deadline() -> None:
    """A row without a deadline cannot expire, and "pending forever" is what the TTL removes.

    Required rather than nullable, so the expiry sweep covers the whole table by
...
def test_decided_at_is_nullable_and_is_the_only_mutation_timestamp() -> None:
    """Exactly one pending → decided transition, so one stamp records it.
...
def test_the_link_to_a_run_lives_here_and_only_here() -> None:
    ...
    deliberately has no `action_request_id` pointing back — two columns describing one edge can
    disagree, and `test_tool_runs_schema.py` records the same decision from its side.
    """
...
def test_both_foreign_keys_set_null_rather_than_cascading() -> None:
    ...
    and the receipts hang off this row, so a cascade would let one user deletion erase
    both the record of what was authorised and the receipt proving it ran. Fails closed
    against requester-match: NULL matches no caller, so a requester-match lookup 404s.
    """
...
def test_the_sweep_and_the_inflight_cap_are_indexed(column_name: str) -> None:
    """The expiry sweep and the per-user in-flight cap are the only recurring queries.
...
def test_the_sweep_index_leads_with_status() -> None:
    ...
    Led by `expires_at` instead, the index would not serve a status-only lookup and the
...
def pending_request(**overrides: object) -> ActionRequest:
    """A gate-time row, with only the required fields filled."""
...
async def test_migration_created_the_check_constraint_on_status(session: AsyncSession) -> None:
    ...
    sit in a status no rule covers.
    """
...
async def test_the_gate_row_defaults_to_pending(session: AsyncSession) -> None:
    """Inserted with no status named, the row still reads PENDING.
...
async def test_approval_context_round_trips_the_gate_time_payload(session: AsyncSession) -> None:
    """Persisted at gate time, read back unchanged when the card renders.

    The payload shape is the gate's, not this column's — what is asserted here is only that
    JSONB gives back what the gate put in, so the provenance and the in-process
...
async def test_approval_context_cannot_be_omitted(session: AsyncSession) -> None:
    """No default, so a card with nothing on it is a failed insert, not a stored row."""
    session.add(
        ActionRequest(
...
async def test_a_request_cannot_be_created_without_a_deadline(session: AsyncSession) -> None:
    """ "Pending forever" is unrepresentable, which is what makes the sweep total."""
...
async def test_an_approval_records_reason_decision_time_and_run(session: AsyncSession) -> None:
    """The full APPROVED shape survives a round trip.
...
async def test_a_pending_request_can_expire_without_a_reason(session: AsyncSession) -> None:
    """An expiry is terminal, and nobody typed anything.
...
async def test_database_refuses_a_decided_row_without_a_reason(
    ...
    """The decision endpoints' rule, closed at the mechanism.

    The 409 lives in the endpoint; this is the same rule at the mechanism, so a *second*
    writer — the executor, the sweep, an admin script — cannot record a decision nobody
...
async def test_an_undecided_row_may_carry_no_reason(
    ...
    answer — the sweep must stay able to write one with `reason IS NULL`. A constraint
