...
async def test_a_refused_backend_beside_a_silent_one_still_names_the_silent_one(
    ...
    csf refused its command and Imunify answered the confirming read with nothing readable. The
    refusal is what the envelope reports, and the silent backend is *still named* beside it —
    a source that cannot answer gets named, and a code that happens to come from the other
    backend is not a substitute for saying which one went quiet.

    No verdict is stated either, and that is the asymmetry rather than caution: with one source
    silent there is no combined reading to hold the refusal up against, so this stays an unknown
    outcome rather than becoming a measurement.
    """
    fixture, _ = release_context(
        monkeypatch,
        box=FakeFirewallBox(
