...
ciphertext on the row, the real `ProxmoxClient`, the real failure normalisation, real
`sanitize_tool_errors`. Only the HTTP socket is doubled.

**What is new on this side, beyond mirroring T22-T26.**

1. **The tool must not be able to name a password**, and that is asserted on the *schema* rather
   than on behaviour (C8, §V.15, §V.43, §V.49). A reason parameter and a `new_password` parameter
   are two spellings of the same mistake: a value the LLM authors reaching a CHANGE.
2. **There is no no-op**, unlike T22/T23/T26. A password reset has no already-in-that-state
...
3. **A `ciuser` mismatch is refused rather than gated.** The password would be set for `ciuser`
   ...
   could.
4. **The preflight names what it could not read** (§V.86). The run state is the one tolerated
...
async def test_an_ambiguous_server_returns_choices_rather_than_a_pick() -> None:
    """§V.18 on a CHANGE path: a guess resets a password on a machine nobody named."""
    fixture, vm = reset_context(
...
async def test_a_well_formed_call_opens_a_pending_request_and_changes_nothing() -> None:
    """§V.16, §V.23: the whole point of the CHANGE gate, asserted on both halves.
...
async def test_the_evidence_carries_the_preflight_state(caplog: pytest.LogCaptureFixture) -> None:
    """§V.33, §V.35: the card describes the VM the operator is authorising a change to.
...
async def test_the_preflight_reads_are_trimmed_before_they_reach_the_evidence() -> None:
    """A pasted node name is an operator artifact, and the runner reads this back later (§V.33).
...
async def test_an_unreadable_run_state_is_named_rather_than_dropped() -> None:
    """§V.86: the one tolerated read says so when it could not answer.
...
async def test_a_required_preflight_read_that_fails_refuses_the_change(vm: FakeProxmoxVM) -> None:
    """§V.35: a card that cannot describe what it is asking about must not be opened.
...
async def test_a_proxmox_permission_failure_keeps_the_code_that_names_the_fix() -> None:
    """§V.19 passes an integration code through rather than flattening it.
...
async def test_the_matching_cloudinit_user_is_not_refused() -> None:
    """The negative control (§V.87): the guard separates rather than refusing everything.
...
# --- C8: the schema, not just the behaviour ---
...
async def test_the_tool_schema_carries_no_password_parameter() -> None:
    """C15, §V.49 at the same surface, and this is the `noa-old` bug (GH #91) restated.
...
async def test_the_recorded_arguments_are_what_the_model_asked_for() -> None:
    """§V.33: the card shows what was requested, and nothing about a password.
...
async def test_the_gate_response_carries_the_text_first_then_the_card() -> None:
    """§V.24, §V.25: both halves ship, in that order, asserted on the count.
...
async def test_the_card_url_carries_the_request_id_and_nothing_else() -> None:
    """§V.26: the tool result persists in LibreChat's MongoDB, so everything in it is readable.

    No node, no VM id, no username, no server name in the URL — the address is a name, and
    authorisation to read the card behind it is the cookie plus §V.27's requester-match.
    """
    fixture, _ = reset_context()
...
async def test_no_operational_detail_reaches_the_transcript() -> None:
    """§V.26, §V.8: not the credentials, and not the VM's state either.
...
async def test_an_unexpected_failure_is_sanitized(monkeypatch: pytest.MonkeyPatch) -> None:
    """§V.19: a raw exception never reaches the model, and a timeout keeps its own name."""
