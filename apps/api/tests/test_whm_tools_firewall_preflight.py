...
**Both backends, in parallel, and only the usable ones** (§V.57). Zero usable backends is an
error and never an empty success; a denied `sudo -n` gets its own code, because "no firewall
...
**Truncated evidence states its bound** (§V.85). `csf -g` on a busy box is hundreds of lines and
...
# --- V57: both backends, only the usable ones, in parallel ---
...
async def test_both_backend_queries_are_in_flight_at_once(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    """`asyncio.gather`, ⊥ two sequential awaits (§V.57).
...
async def test_zero_usable_backends_is_an_error_not_an_empty_success(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    """§V.57. A success carrying `not_found` here would say the address is clean on a server
...
async def test_a_block_on_either_backend_wins_over_an_allow(  # type: ignore[no-untyped-def]
    ...
    statement is "still blocked" — T25 releases *and* allows in one action, so that intermediate
    state is real.
    """
    fixture, _ = firewall_context(
        monkeypatch,
...
async def test_unreadable_csf_output_is_not_an_answer_either(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    A parse regression must ⊥ read as a clean address, so `unknown` counts as not having
...
# --- V19: a failed backend keeps the code that names its remedy ---
...
# --- V54: the preflight accepts every kind the CHANGE tools refuse ---
...
async def test_the_preflight_answers_for_every_target_kind(  # type: ignore[no-untyped-def]
    ...
    """§V.54 belongs to the CHANGE tools, which write rules and take IPv4 only.
...
async def test_the_target_is_trimmed_but_never_normalised(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    distinction the CHANGE tools check (§V.54)."""
    fixture, fake = firewall_context(monkeypatch)
...
async def test_a_blank_target_is_refused_before_any_io(monkeypatch, target: str) -> None:  # type: ignore[no-untyped-def]
    """§V.21. `csf -g ""` matches every line in the tables."""
    fixture, fake = firewall_context(monkeypatch)
...
async def test_an_ambiguous_server_ref_returns_choices(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    """§V.18: a tie is candidates, never a pick — a guess would read the wrong firewall."""
...
# --- C7, V48, V82: the credential path and the pin ---
...
async def test_the_ssh_hop_uses_the_decrypted_credentials_and_the_pin(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    only in production — and the pin travelling is what `ssh_exec` enforces (§V.82).
    """
    fixture, fake = firewall_context(monkeypatch)
...
async def test_the_database_session_closes_before_the_first_ssh_hop(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    """T21's rule, and the reason the SSH layer takes a config rather than a row.
...
# --- V85: capped evidence carries its own bound ---
...
async def test_the_result_carries_no_raw_command_output(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    """DECISIONS §6.5 (old V75): the `csf.deny` log line, never a raw iptables dump.

    `noa-old` shipped `raw_output` and `raw_data` beside the parsed verdict, and the result
    persists in LibreChat's MongoDB (§V.26).
    """
    table_dump = (
...
# --- V19: an exception is a named failure ---
