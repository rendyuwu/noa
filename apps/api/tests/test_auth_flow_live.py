...
accept every password and erase the failure cases V9 needs.
...
# --- V7: provisioning across a real transaction ---
...
def test_first_login_persists_the_provisioned_row_despite_the_403(
    ...
    """V7: the row an admin has to enable is durable before the error propagates.
...
def test_bootstrap_admin_gets_the_admin_role_row(
    ...
    """V7's break-glass, verified as actual `roles` / `user_roles` rows."""
    client, _ = live_client
...
def test_repeat_logins_do_not_duplicate_the_user_or_the_role(
    ...
    """Every login re-provisions and re-assigns; unique constraints ⊥ be tripped."""
    client, _ = live_client
...
# --- V6: the per-request re-read, against real SQL ---


def test_disabling_the_row_ends_the_live_session_on_the_next_request(
    live_client: tuple[TestClient, FakeDirectory], database_url: str
) -> None:
    """V6 keystone: the cookie is unchanged and still valid — only the row moved.
...
def test_deleting_the_row_invalidates_the_session(
    ...
    """A signed cookie for a deleted operator is not a session (401, ⊥ 500)."""
    client, _ = live_client
    login(client)
    _set_active(database_url, OPERATOR_EMAIL, is_active=True)
    login(client)
...
# --- V9: counters across a real rollback ---
...
def test_successful_login_clears_the_persisted_counters(
    ...
    """V9 counts guesses, so a proven operator leaves no bucket behind."""
    client, _ = live_client
    _execute(
        database_url,
        "INSERT INTO users (email, is_active) VALUES (:email, true)",
...
def test_logout_needs_no_database_round_trip(
    ...
    """V6: idempotent, unauthenticated, and no session dependency to fail on."""
    client, _ = live_client
