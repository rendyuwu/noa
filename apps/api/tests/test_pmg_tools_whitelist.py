"""`pmg_whitelist` — one tool for two directions (T29 — §V.58, §V.59, §V.60, §V.61).

The tool half: the guards, the enum, the in-process preflight, the two no-ops, the gate response
and the mount. Its runner lives on the far side of §V.22's boundary and is asserted in
`test_pmg_whitelist_runner.py`; the two are separate files because together they run past C14's
line budget, and the split falls on the boundary the design already draws — **nothing here can
...
names. The seams are T21's: the real resolver, a real `SecretCipher` with real ciphertext on the
row, the real `pmgsh` command composition, the real `mynetworks` parser, real
`sanitize_tool_errors`. Only the SSH socket is doubled.

**What is new on this side, beyond mirroring T22-T28.**

1. **The enum is the tool** (DECISIONS §9). One name where `noa-old` had two, and `action` is
...
2. **§V.59 bites harder than it does on the search tool.** A masked target is a request about a
...
# write — about the network, which is the whole of §V.59's warning on a CHANGE.
MASKED_TARGET = "203.0.113.10/24"
MASKED_NETWORK = "203.0.113.0/24"


def evidence_of(fixture: Any) -> dict[str, Any]:
    """The preflight the gate persisted on the one row it opened (§V.33)."""
...
def registered_tool(fixture: Any) -> Any:
    ...
    `run_middleware=False`: the RBAC gate reads a caller off the request context (§V.1), and every
...
async def test_a_malformed_call_is_refused_before_any_command(
    ...
    directly, bypassing pydantic (T25's three-place discipline).
...
async def test_an_ambiguous_server_returns_choices_rather_than_a_pick(
    ...
    """C10, §V.18: a tie is `choices`, never a guess — whitelisting the wrong gateway is a change
...
# --- DECISIONS §9: one tool, and the enum is on the published schema ---


async def test_one_tool_carries_the_action_enum_and_the_two_old_names_are_gone(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """DECISIONS §9's second collapse, asserted where a model actually reads it.
...
async def test_the_tool_schema_carries_no_reason_parameter(
    ...
    """C8, §V.15, §V.43 at the surface the LLM actually sees.

    The reason is typed by an operator on the card. A parameter of any of these names would ask
    the model to author one, which is the thing C8 forbids — and the schema is where a model
    learns what it may say.
    """
    fixture, _ = whitelist_change_context(monkeypatch)
    server = registered_tool(fixture)
...
async def test_the_tool_is_registered_as_a_change_in_the_catalog(
    ...
    """§V.20, §V.83: the risk is declared at registration, and the name is one grants can name.
...
async def test_no_whitelist_preflight_is_exposed_as_a_tool(
    ...
    """C9, §V.17: the preflight runs in-process inside the CHANGE call, and is not callable.
...
async def test_a_whitelist_already_in_the_asked_for_state_opens_nothing(
    ...
    """There is nothing for an operator to authorise, so no card is opened (T22, T26, T28's shape).

    The `/32` case is §V.59 in the branch that decides whether anything happens at all: an
    operator who typed `203.0.113.10` against a whitelist holding `203.0.113.10/32` is asking
    about one entry, and comparing raw strings would open a card to add a line that is there.
    """
...
async def test_the_no_op_answer_is_built_from_facts_and_not_from_pmgsh_output(
    ...
    """§V.26: this answer is a plain tool result, so it lands in the transcript LibreChat keeps.

    Built from the node's name, the two spellings of the target and one measured boolean — T26's
    rule one system over. A `pmgsh` line, an id column or a neighbouring entry appearing here
    would be the whitelist of a mail gateway sitting in a conversation nobody bounded.
    """
...
async def test_a_masked_no_op_names_the_network_rather_than_the_address(
    ...
    """§V.59: `203.0.113.10/24` is a question about `203.0.113.0/24`.
...
async def test_a_containing_network_is_not_a_match(monkeypatch: pytest.MonkeyPatch) -> None:
    """§V.59's other half, and the negative control for the no-op above.
...
# --- §V.16, §V.23: it opens a question and changes nothing ---
...
async def test_a_well_formed_call_writes_nothing_and_opens_one_pending_request(
    ...
    """The whole of §V.16 on this tool, asserted from both sides.
...
async def test_the_row_carries_the_preflight_the_operator_will_see(
    ...
    """§V.33, §V.35: born in-process, persisted on the row, never rebuilt from a transcript.
...
async def test_the_evidence_carries_no_raw_command_output(
    ...
    """§V.26, §V.93's shape: a structure with nowhere to put the whole whitelist cannot leak it.
...
async def test_the_arguments_on_the_row_carry_no_reason(monkeypatch: pytest.MonkeyPatch) -> None:
    """C8 at the third place it can leak: what the gate recorded, not just what was declared."""
    fixture, _ = whitelist_change_context(monkeypatch)
...
# --- §V.19: what a failure looks like in front of a model ---


async def test_an_unreadable_whitelist_is_a_refusal_rather_than_a_card(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """One source, so §V.86 has no partial case — a read that cannot answer is fail-closed.
    ...
    is the state §V.35 exists to prevent.
    """
    fixture, _ = whitelist_change_context(
        monkeypatch,
...
# --- §V.24, §V.25, §V.26: the gate response ---
...
async def test_the_approval_url_carries_the_request_id_and_nothing_else(
    ...
    """§V.26: this string persists in LibreChat's MongoDB, so everything in it is readable there.
...
async def test_no_secret_from_the_server_row_reaches_the_answer(
    ...
    """§V.8 on the surface that becomes a transcript."""
    fixture, _ = whitelist_change_context(monkeypatch)
