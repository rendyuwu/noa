...
A WHM row may hold a *reseller* token rather than the root one, and a
reseller token may only write the accounts it owns (measured on a live host). Two
things read this column, and neither is an authorization check:

- `whm_list_servers` filters its output by it. 16 clusters times ~7 rows is a
  context problem, not a permission problem.
- the admin write refuses a `true` row whose `name` is not its `api_username`,
...
There is no backfill — `false` is the truthful value for a root credential, and the 16
...
# after the DDL, on the bookkeeping UPDATE.
