...
1. **A tool returns a structured result; it does not raise at the model.** Success is
   ...
   Ported from `noa-old`, and it is what V18 requires — an ambiguous identifier is a
   result the model can act on, not an exception. `sanitize_tool_errors` is what makes
   the promise total.
...
generates and the model never sees, C15/V49). Still to come: `pmg_whitelist` and
`proxmox_vm_nic`.
