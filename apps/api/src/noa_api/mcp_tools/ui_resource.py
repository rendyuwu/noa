"""The two surfaces NOA renders inside LibreChat's frame, shaped once (T32, T56 — V64).

Two tool results carry a NOA-origin document: a CHANGE's approval card (T32) and a large
READ's table (T56). V64 says the table's mechanism is *the same* one V37 names — path A, the
...
- `text/uri-list` is what mcp-ui maps to `externalUrl` -> `iframeRenderMode: 'src'`, which is
  ...
  `srcDoc` on an opaque origin, where the session cookie cannot follow (C17);
...
def embed_url(*, embed_base_url: str, path: str, identifier: str) -> str:
    ...
    tool result that persists in LibreChat's MongoDB (V26), so authorisation to read what is
...
def build_ui_resource(*, uri: str, url: str) -> EmbeddedResource:
    """The iframe half of a tool result, in the shape the render gate measured (T59, R29)."""
