"""The owner-vs-credential gate for WHM account changes.

**cPanel gates an account write on ownership, not on the token's ACL.** Measured on a live host
rather than read off a doc: a root token with `suspend-acct` granted and `all` *absent*
...
remember, which is the shape a guard is eventually written without — one helper, not two, and
`firewall_gate`'s argument one system over.
...
of the unsuspend tool's locked-suspension bound one field over, and the difference is evidence
rather than
...
present on 451 of 451 rows across 7 owners, never blank. So failing closed here costs
nothing in measured reality, and failing open buys exactly the harm owner-as-`server_ref` exists
to prevent: a
...
**Four fields, and the token is not one of them.** A privileged write whose credential is
...
# already on the evidence as `EVIDENCE_SERVER_NAME`; these are the other three recorded fields.
# Read by the
...
class Ownership(Enum):
    """Whether the credential is **proven** to own the account — zero answers means `unknown`.
...
def classify_ownership(*, owner: object, api_username: object) -> Ownership:
    """Which of the four ownership verdicts these two names produce.

    Both sides through `normalize_whm_identity`, which is the whole of the owner compare's
    normalisation and is shared with the admin-write guard's name == `api_username` rule: WHM
    echoes an owner as it was typed and an
...
def recorded(value: object) -> str:
    """One of the four recorded fields as a card shows it, or the word for a non-answer."""
...
def refuse_unproven_ownership(
    *,
    ...
    """Log an unproven-ownership refusal and shape it, for both sites that make it.
    ...
    **The sentence echoes `server_ref` — what the caller passed — and never the resolved row's
    `name`.** The admin-write guard forces a reseller row's `name` to *be* its `api_username`,
    and the server-list tool keeps those rows out of its output, so printing the resolved name
    would disclose a
    ...
    The owner *is* named — the model read it off `whm_search_accounts`, and the owner compare
    needs it to
    ...
    `owner=root` accounts (measured on a live host) the machine's own row is the answer, so the
    sentence states the
