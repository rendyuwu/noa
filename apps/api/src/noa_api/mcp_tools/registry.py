"""The one place a tool name becomes callable over MCP (T19 — I.mcp, V10).

`build_mcp_server` calls this and nothing else registers tools. That single entry point is
what makes two properties checkable instead of hoped for:
...
`server.list_tools()` names equal this return value, so a registrar that under- or
over-reports fails there.
...
def register_mcp_tools(server: FastMCP, *, context: McpToolContext) -> frozenset[str]:
    """Register every exposed tool on `server`; return the names registered (I.mcp)."""
    registered = frozenset(register_whm_read_tools(server, context=context))
    assert_names_in_catalog(registered)
    return registered
