...
class WHMClient:
        ...
        try:
            async with httpx.AsyncClient(
                ...
                response = await client.get(
                    url, params=merged_params, headers=self._headers()
                )
    ...
    async def _get_text(
        ...
        try:
            async with httpx.AsyncClient(
                ...
                response = await client.get(
                    url, params=dict(params or {}), headers=self._headers()
                )
        except httpx.TimeoutException:
            return {
                "ok": False,
    ...
    async def change_contact_email(
        ...
        # modifyacct is the standard WHM API call for account modifications.
        result = await self._get_json_api(
            "modifyacct",
