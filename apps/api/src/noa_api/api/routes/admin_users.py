...
`noa_api.api.errors` owns status, body and `request_id`, so the same condition cannot
...
- internal `user:` roles. The panel renders `roles` as the assignable set and PUTs
...
class AdminUserResponse(BaseModel):
    """One user as the admin panel reads them.

    `tools` is the *effective* set the RBAC engine resolved for this read: every known tool for
    an admin, empty for a disabled account whatever its roles say. A snapshot for a
    body, never a permission cache — the execution gate re-resolves per call.
    """
...
class UpdateUserRequest(BaseModel):
    ...
    directory on every login, so editing them here would write a value the next bind
...
class SetUserRolesRequest(BaseModel):
    ...
    (400 `internal_role_forbidden`) and preserved — the caller neither sends them nor
    can clear them.
    """
...
async def list_users(
    ...
    """Every user with their roles and effective tools.

    Read fresh on every call, with no cache anywhere behind it: what this shows is what the
...
async def update_user_active(
    ...
    Refusals: 404 `admin_user_not_found`, 409 `last_active_admin`, 409
    `self_deactivate_admin` when an admin aims at their own account — a refusal nobody
...
async def set_user_roles(
    ...
    """Replace one user's assignable roles.
...
async def set_user_tools(
    ...
    """Direct per-user tool grants are gone: 410 `direct_tool_grants_disabled`.
    ...
    **No `AuthorizationServiceDep` either.** Nothing is looked up: `user_id` is not checked for
    ...
    `AdminUserDep` stays, so `require_admin` runs first and a non-admin gets 403. Ordering
    ...
    panel reads names it. The body is the shared envelope — the raise is what produces
