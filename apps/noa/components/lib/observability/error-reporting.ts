...
import { ApiError } from "@/components/lib/http/fetch-client";

type ReportExtraValue = boolean | number | string;

export type ErrorContext = Record<string, ReportExtraValue | null | undefined>;
...
const addExtra = (extras: Record<string, ReportExtraValue>, key: string, value: unknown) => {
...
export function shouldReportClientError(error: unknown) {
...
export function buildReportExtras(error: unknown, context: ErrorContext = {}) {
