...
const loadErrorReportingProvider = async (): Promise<ComponentType<{ children?: ReactNode }>> => {
...
describe("ErrorReportingProvider", () => {
  ...
  it("registers global listeners on mount and removes them on cleanup", async () => {
    ...
    const rejectionHandler = addEventListenerSpy.mock.calls.find(([type]) => type === "unhandledrejection")?.[1];
