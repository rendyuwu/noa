...
describe("convertAssistantState", () => {
  ...
  it("ensures tool-call parts always have argsText", () => {
    ...
    const toolPart = (message as any)?.content?.find?.((part: any) => part?.type === "tool-call");
  ...
  it("merges tool-result messages onto the matching tool-call part", () => {
    ...
    const toolPart = (converted.messages[0] as any).content.find((part: any) => part.type === "tool-call");
  ...
  it("marks the latest assistant message as running while transport is sending", () => {
    ...
    expect(converted.messages[0]?.status).toEqual({ type: "running" });
  });
});
